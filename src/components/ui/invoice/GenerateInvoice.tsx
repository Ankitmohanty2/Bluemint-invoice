...
const GenerateInvoice = () => {
  ...
  const filenameRef = useRef("Invoice.pdf");

  const onInvoiceGenerate = () => {
    let finalName = invoiceName.trim();
    if (!finalName) finalName = "Invoice";
    
    filenameRef.current = finalName.endsWith(".pdf") ? finalName : `${finalName}.pdf`;
    ...
    <main className="max-w-7xl px-6 mx-auto mt-10">
      <div className="border-2 shadow-lg px-4 mb-8 rounded-xl">
        <div className=" md:my-6 my-4 flex flex-col gap-7 rounded">
          ...
          <InvoiceFooter totalWithTax={totalWithTax} onInvoiceGenerate={onInvoiceGenerate} customMessage={customMessage} setCustomMessage={setCustomMessage} invoiceName={invoiceName} setInvoiceName={setInvoiceName} />
        </div>
      </div>
    </main>
  );
